...
- Network: zaawansowany skaner TCP pojedynczego adresu lub podsieci CIDR, pola oktetów z kropkami i maską, opcjonalne skanowanie otwartych portów, ICMP, DNS/FQDN, nazwa komputera, MAC, heurystyczne rozpoznanie drukarki/komputera/routera/kamery/innego urządzenia, szczegóły hosta oraz eksport JSON/CSV/PDF. Wyniki hostów są dopisywane do tabeli na bieżąco, bez czekania na zakończenie całej podsieci. Menu prawokliku udostępnia RDP, OfferRA, kopiowanie danych i eksport. Dla wykrytych komputerów Windows dostępny jest RDP, a pomoc techniczna uruchamia wyłącznie `msra.exe /offerRA` z ręcznie wpisanym adresem IP.
